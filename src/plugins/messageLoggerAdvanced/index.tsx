...
import { Modals, ModalAPI } from "@utils/modal";
import { useState, Forms } from "@webpack/common";

const { ModalRoot, ModalHeader, ModalContent, ModalFooter, ModalCloseButton } = Modals;

function EditMessageModal({ rootProps, message, channelId, onSave }) {
    const [newContent, setNewContent] = useState(message.content);

    return (
        <ModalRoot {...rootProps} key={'ml-toggle-style'} aria-label="Edit Message">
            <ModalHeader>
                <Forms.FormTitle tag="h2">Edit Message</Forms.FormTitle>
...
function openEditMessageModal(message, channelId) {
    ModalAPI.openModal(
        (props) => (
            <EditMessageModal
                rootProps={props}
                ...
                onSave={(newContent: string) => {
                    if (newContent !== message.content) {
                        updateMessage(channelId, message.id, { content: newContent });
                    }
                }}
            />
        ),
        { onCloseRequest: () => ModalAPI.closeModal('ml-toggle-style') }
    );
}

const REMOVE_HISTORY_ID = "ml-remove-history";
const TOGGLE_DELETE_STYLE_ID = "ml-toggle-style";
const EDIT_MESSAGE_ID = "ml-toggle-styleo";
const DEL_MESSAGE_ID = "ml-toggle-stylee";
const patchMessageContextMenu: NavContextMenuPatchCallback = (children, props) => {
    ...
    children.push(
        <Menu.MenuItem
            ...
            label="Edit Message"
            action={() => {
                openEditMessageModal(message, channel_id);
            }}
        />
    );
...
export default definePlugin({
    name: "MessageLoggerAdvanced",
    description: "Temporarily logs deleted and edited messages.",
    authors: [Devs.rushii, Devs.Ven, Devs.AutumnVN, Devs.Nickyux, Devs.Kyuuhachi, { name: "Jumble", id: 1135976262234542233n }],
